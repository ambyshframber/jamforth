...
// Encoder MUST be fully zeroed entering this function.
extern void qoi_enc_init(struct qoiencoder* q, void* buf, int w, int h) {
    q->c = 0xff000000;
    unsigned char* p = buf;
...
extern void qoi_enc_alpha(void* buf) {
    unsigned char* p = buf;
    p[12] = 4;
}
...
extern int qoi_encode(struct qoiencoder* q, void* buf, unsigned c)
{
    unsigned char* p = buf;
    if (q->c == c) {
        if (++q->run == 62) {
...
extern int qoi_enc_finish(struct qoiencoder* q, void* buf)
{
    unsigned char* p = buf;
    if (q->run) {
        *p++ = 0xc0 | (q->run - 1);
